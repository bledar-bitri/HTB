...
function UNI_date2regexp(txt) {
	...
	txt = txt.replace(/tt/g, '(AM|PM|am|pm)*');
...
function UNI_validateDate(el, props) {
	...
	var re = UNI_date2regexp(dateMask);
	var arr = re.exec(el.value);
	if (!arr) {
		toret = false;
	} else {
		var o = UNI_parse_date(arr, dateMask);
...
function UNI_validate_minmax_date(el, props) {
	...
	if (props['max'] != '') {
		...
		var date_max = UNI_parse_date(arr, dateMask);
...
//TODO : redo insert / update / delete logic ??? 
function UNI_attachToButtons(frm) {
	if (is.ie && is.mac) {
		return;
	}
	for (var i = 0; frm.elements.length; i++) {
			...
			if (el.name) {
				...
				if (el.name.toString().match($UNI_DELETE_BUTTON_NAME) 
				 || el.name.toString().match($UNI_CANCEL_BUTTON_NAME)
					...
					attached = true;
					utility.dom.attachEvent(el, 'focus', UNI_buttonHandler, 0);
				}
				if (el.name.toString().match($UNI_INSERT_BUTTON_NAME)
				 || el.name.toString().match($UNI_UPDATE_BUTTON_NAME)
				 ) {
					attached = true;
					utility.dom.attachEvent(el, 'focus', UNI_buttonHandler, 1);
				}
...
function UNI_attachToForm() {
	...
	for (var i = 0; i < frms.length; i++) {
		var frm = frms[i];
		//utility.dom.attachEvent2(frm, 'submit', function(e) {return UNI_formSubmittalHandler() } , 1);
		if (typeof frm.onsubmit != 'undefined' && frm.onsubmit != null) {
			...
			var f = new Function('e', 'if (!UNI_formSubmittalHandler(e)) return false; ' + str);
		} else {
			var f = new Function('e', 'return UNI_formSubmittalHandler(e);');
		}
		frm.onsubmit = f;
		UNI_attachToButtons(frm);
	}
}
...
function UNI_disableButtons(frm, regexp) {
...
function UNI_formSubmittalHandler(e) {
	...
	var widgets_error = frm.getAttribute('widgets_error');
	if (widgets_error != null && widgets_error.toString() == 'true') {
		returnHandler = false;
	}
	
	if (!returnHandler) {
		frm.removeAttribute('widgets_error');
...
function UNI_handle_required(form_label) {
	...
	if (htmlFor) {
		var el = document.getElementById(htmlFor);
...
function UNI_form_attach() {
	...
	Array_each(start_el.getElementsByTagName('input'), function(button, i) {
		if (button.type && Array_indexOf(['submit', 'button'], button.type.toLowerCase()) >= 0) {
			if (button.className == 'mxw_v') { return; }
