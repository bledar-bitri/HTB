...
function swap_order_fields (s, dir, indexes, newindexes) {
	for (var i = 0; i < indexes.length; i++) {
		...
		s.options[i1].value = tmp2.id + $LIS_VALUES_SEPARATOR + tmp1.order + $LIS_VALUES_SEPARATOR + tmp2.original_order;
		s.options[i1].selected = false;

		s.options[i2].text = tmp1.text;
		s.options[i2].value = tmp1.id + $LIS_VALUES_SEPARATOR + tmp2.order + $LIS_VALUES_SEPARATOR + tmp1.original_order;
