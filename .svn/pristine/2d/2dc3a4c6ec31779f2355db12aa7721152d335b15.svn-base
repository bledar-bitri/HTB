show_as_buttons = "!(typeof $NXT_LIST_SETTINGS == 'undefined' || typeof $NXT_LIST_SETTINGS['show_as_buttons'] == 'undefined' || $NXT_LIST_SETTINGS['show_as_buttons'] == false )";
...
$TRANSFORMATIONS = [
	...
	}, 
	{
		...
		'transform': function(el) {
			...
			if (Array_indexOf(classes, 'KT_moveup_link') >= 0
			||  Array_indexOf(classes, 'KT_movedown_link') >= 0) {
				elnou.className = 'button_smallest';
			} else {
				elnou.className = 'button_big';
			}
	...
	{
		'selector' : 'div.KT_tnglist div.KT_textnav ul li a', 
		'transform' : function(el) {
			var elnou = KT_style_replace_with_button(el);
	...
	}, 
	{
		...
		'transform' : function(el) {
			...
			if (typeof window['ktmls'] != 'undefined' && is.mozilla) {
				return;
			}
