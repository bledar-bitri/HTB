...
namespace HTB.v2.intranetx.aktenint
{
    public partial class ImportKsv : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            bool isCSV;
            var sheetNames = GetExcelSheetNames(fileName, out isCSV);
            var ok = true;
            if (isCSV)
            {
                ok = LoadCSV(fileName);
        ...
        #region XL
        private String[] GetExcelSheetNames(string strFile, out bool isCSV)
        {
            DataTable dt = null;
            isCSV = false;
            strFile = strFile.ToLower();
            if (strFile.Trim().EndsWith(".csv"))
            {
                isCSV = true;
                return null;
            }
...
//            }
        ...
        {
            var engine = new FileHelperEngine<AlektumInkasso>();

            // To Read Use:
            var casses = engine.ReadFile(fileName);
            var ok = true;
            foreach (var alektumInkasso in casses)
            {    
                if (ok)
                    ok &= CreateAkt(alektumInkasso);
                _coutner++;
                if (_coutner % 50 == 0)
                    Thread.Sleep(5000);
            }
        ...
        private bool CreateAkt(AlektumInkasso rec)
        {
            string ksvAuftragsnummer = "";
            try
            {
                ...
                DateTime gegnerDob = GlobalUtilArea.GetDefaultDateIfConvertToDateError(rec.SchuldnerGebDat);
                if (gegnerDob == HTBUtils.DefaultDate)
                    GetDateFromString(rec.SchuldnerGebDat);
                ...
                tblGegner gegner = GetGegner(rec.Name1, rec.Name2, rec.SDStrasse, rec.SDPLZ, gegnerDob) ?? CreateGegner(new tblGegner
                {
                    GegnerName1 = rec.Name1,
                    GegnerName2 = rec.Name2,
                    GegnerName3 = rec.Alias,
                    GegnerStrasse = rec.SDStrasse,
                    GegnerZipPrefix = rec.SDLKZ,
                    GegnerZip = rec.SDPLZ,
                    GegnerOrt = rec.SDOrt,

                    GegnerLastStrasse = rec.SDStrasse,
                    GegnerLastZipPrefix = rec.SDLKZ,
                    GegnerLastZip = rec.SDPLZ,
                    GegnerLastOrt = rec.SDOrt,

                    GegnerGebDat = gegnerDob,
                    GegnerOldID = TmpOldId,
                    GegnerCreateDate = DateTime.Now,
                    GegnerPhone = rec.Telefonnummer,
                    GegnerCreateSB = HTBUtils.GetControlRecord().AutoUserId
                });

                tblKlient klient = GetKlient(rec.Glaeubiger, rec.GLStrasse, rec.GLPLZ) ?? CreateKlient(new tblKlient
                {
                    KlientName1 = rec.Glaeubiger,
                    KlientStrasse = rec.GLStrasse,
                    KlientLKZ = "A",
                    KlientPLZ = rec.GLPLZ,
                    KlientOrt = rec.GLOrt,
                    KlientType = 5,
                ...
                var akt = CreateAkt(new tblAktenInt
                {
                    ...
                    AktIntSB = aktSb,
                    AKTIntDistance = distance,
                    AKTIntKosten = GlobalUtilArea.GetZeroIfConvertToDoubleError(rec.OffeneKSVKosten),
                    AKTIntZinsen = 0,
                    AKTIntZinsenBetrag = GlobalUtilArea.GetZeroIfConvertToDoubleError(rec.OffeneZinsen),
                    AKTIntKSVEMail = rec.SachbearbeiterEmail,
                    AKTIntAGSB = rec.Sachbearbeiter,
                    AktIntOriginalMemo = rec.Forderungstext,
                    AKTIntDub = chkIsDub.Checked ? 1 : 0,
                    AktIntAktType = chkIsDub.Checked ? 4 : 1,
                });
                ...
                CreatePos(akt.AktIntID, "Buchung lt. " + rec.KundennummerVonSDBeimAG, rec.UebergebenesKapital, "Ursprüngl. Forderungen");
                CreatePos(akt.AktIntID, "Buchung lt. " + rec.KundennummerVonSDBeimAG, rec.OffeneAGSpesen, "Spesen Auftraggeber");
                CreatePos(akt.AktIntID, "Buchung lt. " + rec.KundennummerVonSDBeimAG, rec.BezKapital, "Bereits bezahlt", true);
                _lastAktImported = akt.AktIntAZ;
            }
            catch (Exception ex)
            {
