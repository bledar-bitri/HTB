...
function newTable(tName, tAliasName, addPredefLinks){
	...
	if (addPredefLinks && predefLinks.length) {
		for (i=0;i<predefLinks.length;i++) {
			...
			} else if (predefLinks[i].table2 == tAliasName && predefLinks[i].table1 != tAliasName && myQuery.tables.item(predefLinks[i].table1)) {
				var ret =myQuery.tables.item(predefLinks[i].table2).columns.item(predefLinks[i].t2id).link(
					myQuery.tables.item(predefLinks[i].table1).columns.item(predefLinks[i].t1id)
				);
			}
		}
	}
