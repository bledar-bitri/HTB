...
function String_objdeserialize(version_check) {
	...
	if (version_check) {
		...
		var this_version = (this+"").match(/version\s\[([0-9\.]*)\]/)
...
function SQLQuery_serialize () {
	...
	var s = '';
	s += sprintf('/* Created with MX Query Builder - version [%s].*/' + NEW_LINE, this.version);
...
function SQLColumn_isFK() {
	...
	for(var i=0; i<rels.length; i++) {
		var rel = rels.item(i);
		if (rel.table1 == this.table.name
		||  rel.table2 == this.table.name) {
			if ( 
				rel.field1 == this.name
					|| 
				rel.field2 == this.name
				 ) {
				toret =  true;
			}
		}
...
function SQLCondition_serialize(uniq, mode) {
	...
	if (this.test_value != '') {
		s += sprintf('tmp3.test_value = "%s";' + NEW_LINE, this.test_value);
	}
	if (this.string != '') {
