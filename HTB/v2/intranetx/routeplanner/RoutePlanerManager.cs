...
namespace HTB.v2.intranetx.routeplanner
{
    ...
    public class RoutePlanerManager : Task, ISerializable, ITspProgressListener
    {
        ...
        public const int DoneCode = 100;
        public const string FixAddressesMessage = "Fix_Addresses";
        ...
        public static readonly string OpenRouteServiceKey = HTBUtils.GetConfigValue("OpenRouteServiceKey");
        public static readonly string OpenRouteServiceUrl = HTBUtils.GetConfigValue("OpenRouteServiceUrl");
        ...
        public RoutePlanerManager(int userId, TaskStatus taskStatus) : this(userId, "")
        {
...
//            CalcBestTour();
        ...
        public override void Run()
        {
            ...
            try
            {
                ...
                var doneMessage = FixAddressesMessage;
                var doneCode = ErrorCode;
                Log.Info($"Addresses: [{Addresses.Count}] Bad:[{BadAddresses.Count}]");
                ...
                {
                    Log.Info("Serializing RoutePlanerManage");
                    var routeFilePath = GetRouteFilePath(RouteUser, RouteName);
                    FileSerializer<RoutePlanerManager>.Serialize(routeFilePath, this);
                }
                Log.Info($"Updating Progress [{doneCode}, {doneMessage}]]");
                UpdateProgressStatus(doneCode, doneMessage);
        ...
        {
            Run();
            if (eventHandler is IRouteEventHandler handler)
        ...
        public string GetJsCoordinatesForFirstLocation(bool reverseTour)
        {
            return map.GetJsCoordinatesForFirstLocation(BestTour, reverseTour);
        }
        public string GetJsCoordinates(int startPoint, int totalPoints)
        {
            Log.Info("BestTourCount " + BestTour.Count);
            return IsReverse ? map.GetJsReversedCoordinates(BestTour, startPoint, totalPoints) : map.GetJsCoordinates(BestTour, startPoint, totalPoints);
        }
        ...
        public string GetJsOverviewWaypoints(int totalPoints)
        {
            Log.Error("BestTourCount "+BestTour.Count);
            return IsReverse ? map.GetJsReversedOverviewWaypoints(BestTour, totalPoints) : map.GetJsOverviewWaypoints(BestTour, totalPoints);
        }
        ...
        public void ClearBadAddresses()
        {
            BadAddresses.Clear();
        }

        public void ReplaceAddress(int id, string address)
        {
            var addr = GetAddressByID(id);
            if (addr != null)
            {
        ...
        protected void LoadFakeDistances(List<Road> roads = null, bool isTask = false)
        {
            if (roads == null)
                roads = GetRoads();
...
//                    tw.Dispose();
        ...
        public void UpdateProgressStatus(int pct, string message)
        {
            ...
            TaskStatus.Progress = pct;
            TaskStatus.ProgressText = message;
        }

        public void UpdateProgressLongMessage(string longMessage)
        {
            if (TaskStatus != null)
            {
                TaskStatus.ProgressTextLong = longMessage;
            }
        }
            ...
            : base("")
        {
            ...
            TaskStatus = (TaskStatus)info.GetValue("TaskStatus", typeof(TaskStatus));
        ...
        public void GetObjectData(SerializationInfo info, StreamingContext context)
        {
            ...
            info.AddValue("TaskStatus", TaskStatus, typeof(TaskStatus));
    ...
    {
        private static readonly log4net.ILog Log = log4net.LogManager.GetLogger(MethodBase.GetCurrentMethod()?.DeclaringType);
        private static readonly string SpName = "spGetGegnerAddressLatAndLgnByAktID";
        ...
        {
            var stateInfo = (AddressLookupState)state;
            Log.Info("Trying Open Street Map");
            if (LoadCoordinatesFromOpenStreetMap(state)) return;
            
            Log.Info("Open Street Map did not work... trying google");
            if (LoadGeocodeAddressFromGoogleMaps(state)) return;
            Log.Info($"Bad Address [{stateInfo.Address.Address}]");
            AddressLookupStateStaticsAccess.GetAddressStateStatic(stateInfo.UserId).BadAddresses.Add(stateInfo.Address);
            
        }

        public bool LoadCoordinatesFromOpenStreetMap(object state)
        {
            lock (LockerObj)
            {
                var stateInfo = (AddressLookupState)state;
                using (var client = new HttpClient())
                {
                    try
                    {
                        ...
                        if (result.Any())
                        {
                            var coordinates = JsonSerializer.Deserialize<GeoResult[]>(json)[0];
                            SaveGegnerLocation(stateInfo.Address.ID, coordinates.Lat, coordinates.Lon);
                        }
                        else
                        {
                            Log.Info($"No GeoCoordinates from URl: [{url}] Address: [{stateInfo.Address.Address}]");
                            return false;
                        }
                    }
                    catch (Exception ex)
        ...
        private bool LoadGeocodeAddressFromGoogleMaps(object state)
        {
            try
            {
                var stateInfo = (AddressLookupState)state;
                ...
                {
                    Log.Info($"Address not found in google {stateInfo.Address.Address}");
                    return false;
                }
                ...
                Log.Info($"Found address in google {stateInfo.Address.Address}");
                ...
                AddressLookupStateStaticsAccess.GetAddressStateStatic(stateInfo.UserId).Addresses.Add(
                    new City(new AddressLocation(stateInfo.Address, new GeocodeLocation[] { location }), null));
        ...
        private void SaveGegnerLocation(int aktId, string lat, string lgn)
        {
            if (aktId > 0 && !Stop)
            {
                var akt = HTBUtils.GetInterventionAktQry(aktId);
                ...
                {
                    var set = new RecordSet();
                    set.ExecuteNonQuery("UPDATE tblGegner SET GegnerLatitude = " + lat.Replace(",", ".") + ", GegnerLongitude = " + lgn.Replace(",", ".") + " WHERE GegnerID = " + akt.GegnerID);
                }
            }
        }
    }
    ...
    internal class RoadDistanceCalculatorThread
    {
        ...
        private void SetGeocodeDistance(Road road, int tries)
        {
            lock (lockObj)
            {
                ...
                var url = $"{RoutePlanerManager.OpenRouteServiceUrl}?api_key={RoutePlanerManager.OpenRouteServiceKey}&start={fromLongitude},{fromLatitude}&end={toLongitude},{toLatitude}";
                ...
                try
                {
                    using (var client = new HttpClient())
                    {
                        ...
                        var distance = result?.features?[0]?.properties?.segments?[0]?.distance / 1000.0;
                        var duration = result?.features?[0]?.properties?.segments?[0]?.duration;
                        road.Distance = distance ?? 0;
                        road.TravelTimeInSeconds = (long)(duration ?? 0);
                        Log.Info($"Found road: {road.DebuggerDisplay}  {road.From} - {road.To}  [Distance: {road.Distance}]");
                        UpdateDistance(road);
                    }
                }
                catch (Exception e)
                {
                    ...
                    if (!Stop)
                    {
                        ...
                        Log.Error(string.Format(
                                "Could Not Calculate Distance [FROM: {0}, {1}] TO [{2}, {3}] [URL: {4}] " +
                                e.Message,
                                road.From.Location.Locations[0].Latitude.ToString(CultureInfo.InvariantCulture)
                                    .Replace(",", "."),
                                road.From.Location.Locations[0].Longitude.ToString(CultureInfo.InvariantCulture)
                                    .Replace(",", "."),
                                road.To.Location.Locations[0].Latitude.ToString(CultureInfo.InvariantCulture)
                                    .Replace(",", "."),
                                road.To.Location.Locations[0].Longitude.ToString(CultureInfo.InvariantCulture)
                                    .Replace(",", "."),
                                url)
                            , e);
                    }
...
//            }
        ...
        private static void UpdateInfo(World sender, UpdateEventArgs e)
        {
            if (e != null)
            {
                if (e.BestTour != null)
                {
                    MapStateStaticsAccess.GetMapStateStatic(e.UserID).RpManager.UpdateProgressLongMessage(GetTourInfo(e.BestTour, e.UserID));
                }
