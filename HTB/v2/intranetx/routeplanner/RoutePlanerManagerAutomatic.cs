...
namespace HTB.v2.intranetx.routeplanner
{
    ...
    public class RoutePlanerManagerAutomatic : RoutePlanerManager
    {
        ...
        #region Constructors
        public RoutePlanerManagerAutomatic(int userId, TaskStatus taskStatus): base(userId, taskStatus)
        {
...
//            Map.DebugInfo("Addresses count "+Addresses.Count);
                if (BadAddresses.Count == 0)
                {
                    ...
                    var startCity = GetStartingPoint();
                    AddNearestNeighbourTour(startCity, startCity, new List<City>(), tour, 0);
                    UpdateProgressStatus(80, $"Tour Calculated [Road Count: {tour.Count}] ");
                    AddTourCities(tour);
                    if (Cities.Count > 4)
                    {
                        ReCalculateRoute();
