...
namespace HTBReports
{
    public class AuftragReceipt
    {
        ...
        private readonly string _reportHeadingLine1 = HTBUtils.GetConfigValue("Report_HeadingLine1");
        private readonly string _reportHeadingLine2 = HTBUtils.GetConfigValue("Report_HeadingLine2");
        private readonly string _reportCompanyName = HTBUtils.GetConfigValue("Report_CompanyName");
        private readonly string _reportCompanyAddress = HTBUtils.GetConfigValue("Report_CompanyAddress");
        private readonly string _reportCompanyMainCity = HTBUtils.GetConfigValue("Report_CompanyMainCity");
        ...
        private void PrintClientCoverPage()
        {
            ...
            if (klient != null)
            {
                ...
                _writer.print((_lin += _gap), _col + 1600, _reportCompanyMainCity + ", am " + DateTime.Now.ToShortDateString(), 'R');
                _lin += _gap * 5;
        ...
        private void PrintClientPageHeader(ECPPdfWriter writer, bool printHeadings = true)
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                writer.print(50, _col + 1600 + i, _reportHeadingLine1, 'R', BaseColor.BLUE); // give it a bolder look
                writer.setFont("Arial", 16, true, false, false);
                writer.print(120, _col + 1600 + i, _reportHeadingLine2, 'R', BaseColor.BLUE); // give it a bolder look
            }
            if (printHeadings)
            {
                _lin = _startLine;
